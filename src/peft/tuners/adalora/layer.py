    ...
    def switch(self, index_list):
        ...
        with torch.no_grad():
            for active_adapter in self.active_adapters:
                ...
                merge_A[index_list, :] = self.lora_A[active_adapter][index_list, :]
                merge_B[:, index_list] = self.lora_B[active_adapter][:, index_list]
                self.lora_A[active_adapter][index_list, :] = 0
...
class RankAllocator:
    ...
    def __init__(self, model, peft_config, adapter_name):
        ...
        self.p_keep = peft_config.p_keep
        ...
        with torch.no_grad():
            for module_name, module in model.named_modules():
                if isinstance(module, SVDLinear):
                    vector_ipt = {}
                    triplet_ipt = {}
                    n = f"{module_name}.lora_A.{self.adapter_name}"
                    ...
                    n = f"{module_name}.lora_B.{self.adapter_name}"
                    entry_ipt = self._element_score(n)
